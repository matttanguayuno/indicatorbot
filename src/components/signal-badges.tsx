...
export function PctChange({ value }: { value: number | null }) {
  ...
  return <span className={`${color} text-xs font-mono`}>{prefix}{value.toFixed(2)}%</span>;
}

/** Colored RVOL display */
export function RvolBadge({ rvol }: { rvol: number | null }) {
  if (rvol == null) return <span className="text-gray-600 text-xs">—</span>;
  let color = 'text-gray-400';
  if (rvol >= 3) color = 'text-green-400';
  else if (rvol >= 1.5) color = 'text-yellow-400';
  return <span className={`${color} text-xs font-mono`}>{rvol.toFixed(1)}x</span>;
}

/** VWAP status display */
export function VwapStatus({ pctFromVwap }: { pctFromVwap: number | null }) {
  if (pctFromVwap == null) return <span className="text-gray-600 text-xs">—</span>;
  const above = pctFromVwap > 0;
  return (
    <span className={`text-xs ${above ? 'text-green-400' : 'text-red-400'}`}>
...
export function DataStatus({ status }: { status: string | null }) {
  ...
  return <span className="text-yellow-500 text-xs">Partial</span>;
}
...
export function TimeAgo({ date }: { date: string | Date }) {
  ...
  return <span className="text-gray-500 text-xs">{d.toLocaleDateString()}</span>;
}
...
export function FloatDisplay({ value }: { value: number | null }) {
  ...
  if (value >= 1_000_000_000) return <span className="text-xs font-mono">{(value / 1_000_000_000).toFixed(1)}B</span>;
  if (value >= 1_000_000) return <span className="text-xs font-mono">{(value / 1_000_000).toFixed(1)}M</span>;
  return <span className="text-xs font-mono">{(value / 1000).toFixed(0)}K</span>;
}
