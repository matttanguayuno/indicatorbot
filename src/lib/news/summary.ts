/**
 * Core news summary generation logic.
 * Called by:
 *  - POST /api/news/summary (HTTP API)
 *  - instrumentation.ts scheduler (direct import, no HTTP round-trip)
 */
...
export async function generateNewsSummary(): Promise<{
  summary: string;
  symbols: string[];
  generatedAt: string;
}> {
  ...
  if (stocks.length === 0) {
    ...
    return { summary, symbols: [], generatedAt: new Date().toISOString() };
  }
  ...
  if (Object.keys(newsPerSymbol).length === 0) {
    ...
    return { summary, symbols: allSymbols, generatedAt: new Date().toISOString() };
  }
